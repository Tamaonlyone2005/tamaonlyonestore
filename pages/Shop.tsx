...
import { ShoppingCart, ArrowRight, Info, Search, Filter, ArrowUpDown, Clock, Flag, Ticket, ShieldCheck, User as UserIcon } from 'lucide-react';
...
const Shop: React.FC<ShopProps> = ({ user }) => {
  ...
  const [sortOrder, setSortOrder] = useState<'OFFICIAL_FIRST' | 'LOW_HIGH' | 'HIGH_LOW' | 'NEWEST'>('OFFICIAL_FIRST');
  ...
  useEffect(() => {
    const loadData = async () => {
        setLoading(true);
        // getGlobalProducts in storageService guarantees Admin > Member sort initially
  ...
  const filteredProducts = useMemo(() => {
      ...
      if (sortOrder === 'LOW_HIGH') {
          ...
          // OFFICIAL_FIRST (Default)
          // Admin products (no sellerId) go first
          res.sort((a, b) => {
              const aIsAdmin = !a.sellerId;
              const bIsAdmin = !b.sellerId;
  ...
  if (activeProduct) {
     ...
     const isMemberProduct = !!activeProduct.sellerId;
                            ...
                            <div className="flex-1">
                                 <div className="flex flex-wrap gap-2 mb-2">
                                     ...
                                     {activeProduct.sellerName ? (
                                         ...
                                         <span className="px-2 py-0.5 rounded text-[10px] bg-blue-500/20 text-blue-300 font-bold uppercase tracking-wider border border-blue-500/20">OFFICIAL STORE</span>
                                     )}
                                 </div>
                                 ...
                                 <div className="inline-flex items-center gap-2 bg-green-500/10 text-green-400 px-3 py-1.5 rounded-lg border border-green-500/20 text-xs font-bold mb-6">
                                     <Clock size={14}/> Rata-rata proses: {isMemberProduct ? '10-30 Menit' : '5-10 Menit (Otomatis)'}
                                 </div>
               ...
               <div className="lg:col-span-1 space-y-6 animate-slide-up">
                     <div className="bg-[#1e293b] rounded-2xl p-6 border border-white/5 shadow-xl sticky top-24">
                         ...
                         <div className="space-y-6">
                              ...
                              {/* Coupon Input - HIDDEN FOR MEMBER PRODUCTS */}
                              {!isMemberProduct && (
                                  <div>
          ...
          {loading ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {filteredProducts.map(p => (
                      <div key={p.id} onClick={() => navigate(`/shop?product=${p.id}`)} className="cursor-pointer group bg-[#1e293b] rounded-xl overflow-hidden border border-white/5 hover:border-brand-500 transition-all shadow-lg hover:-translate-y-1 relative">
                          <div className="aspect-[4/3] relative overflow-hidden bg-gray-800">
                              ...
                              {/* Type Badge */}
                              <div className="absolute top-2 right-2 bg-black/60 px-2 py-0.5 rounded text-[9px] text-white backdrop-blur-md border border-white/10">
                                  {p.type}
                              </div>

                              {/* OFFICIAL / SELLER TAGS */}
                              {!p.sellerId ? (
                                  <div className="absolute top-2 left-2 bg-blue-600 px-2 py-0.5 rounded text-[9px] text-white font-bold flex items-center gap-1 shadow-lg border border-blue-400/50">
                                      <ShieldCheck size={10} className="fill-current"/> OFFICIAL
                                  </div>
                              ) : (
                                  <div className="absolute top-2 left-2 bg-purple-600 px-2 py-0.5 rounded text-[9px] text-white font-bold flex items-center gap-1 shadow-lg border border-purple-400/50">
                                      <UserIcon size={10} className="fill-current"/> SELLER
                                  </div>
                              )}
                          </div>
                          
                          <div className="p-3">
                              <h4 className="text-white text-xs md:text-sm font-bold truncate mb-1" title={p.name}>{p.name}</h4>
                              <div className="flex items-center justify-between">
                                  <p className="text-brand-400 text-xs md:text-sm font-black">Rp {p.price.toLocaleString()}</p>
                                  {p.isBoosted && <span className="text-[8px] bg-yellow-500/20 text-yellow-500 px-1 rounded">AD</span>}
                              </div>
                              {p.sellerName && <p className="text-[9px] text-gray-500 truncate mt-1">Tok: {p.sellerName}</p>}
                          </div>
                      </div>
                  ))}
              </div>
